...
labelme_data_path = 'labelme-dataset/'
...
def perturb_with_occl(inp_imgs, n_segs, fname_idx_start, occl_perturbed_imgs_path, final_perturbed_path, output_res):
  ...
  labelme_images_path = [(labelme_data_path+f) for f in os.listdir(labelme_data_path) if os.path.isfile(os.path.join(labelme_data_path, f))]
  ...
  for labelme_image_rand_path in labelme_images_path_rand:
    ...
    fname = os.path.join(occl_perturbed_imgs_path, "img" + '_ol_' + '{0:06d}'.format(fname_idx_start) + ".jpg")
    # cv2.imshow("Light perturbed image", pl_img)
    cv2.imwrite(fname, target_img)

    fname_final = os.path.join(final_perturbed_path, "img" + '{0:06d}'.format(fname_idx_start) + ".jpg")
    # cv2.imshow("Light perturbed image", pl_img)
